    ...
    <div className="resume-paper" style={{
      ...
      <aside style={{
        ...
        <div data-section="personal">
          ...
          {personal.title && (
            <p style={{ fontSize: '0.6563em', color: accent, textTransform: 'uppercase', letterSpacing: '0.12em', marginTop: '5px', marginBottom: 0 }}>
              {personal.title}
            </p>
          )}
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {skills.map(s => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                  <span style={{ fontSize: '0.6875em', color: 'rgba(255,255,255,0.88)', fontWeight: 500 }}>{s.name}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {languages.map(l => (
                <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '6px' }}>
                  <span style={{ fontSize: '0.6875em', color: 'rgba(255,255,255,0.88)', fontWeight: 500 }}>{l.language}</span>
                  {l.proficiency && (
                    <span style={{
                      backgroundColor: 'rgba(255,255,255,0.12)', color: accent, fontSize: '0.5938em',
          ...
          <ForestSideSection title="Certifications" accent={accent} sectionId="certifications">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {certifications.map(c => (
                <div key={c.id}>
                  <div style={{ fontSize: '0.6875em', color: 'rgba(255,255,255,0.88)', fontWeight: 600 }}>{c.name}</div>
                  {c.issuer && <div style={{ fontSize: '0.625em', color: 'rgba(255,255,255,0.45)', marginTop: '1px' }}>{c.issuer}</div>}
                  {c.date && <div style={{ fontSize: '0.625em', color: 'rgba(255,255,255,0.45)' }}>{c.date}</div>}
                </div>
              ))}
            </div>
          </ForestSideSection>
              ...
              {experience.map(exp => (
                <div key={exp.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3px' }}>
                    <div>
                      <div style={{ fontSize: '0.8438em', fontWeight: 700, color: '#111827' }}>{exp.role}</div>
                      <div style={{ fontSize: '0.7188em', fontWeight: 600, color: primary, marginTop: '1px' }}>{exp.company}</div>
                    </div>
                    <span style={{
                      backgroundColor: '#F0FDF4', color: '#166534', fontSize: '0.625em',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {projects.map(p => (
                <div key={p.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  ...
                  {p.description && <RichContent html={p.description} style={{ fontSize: '0.75em', color: '#4B5563', margin: '0 0 4px', lineHeight: 1.6 }} />}
                  {p.tech.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                      {p.tech.map((t, i) => (
                        <span key={i} style={{ backgroundColor: '#ECFDF5', color: '#065F46', fontSize: '0.625em', padding: '2px 7px', borderRadius: '4px', fontWeight: 600 }}>{t}</span>
                      ))}
                    </div>
                  )}
                </div>
...
const ForestContactRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div>
    <div style={{ fontSize: '0.5625em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.4)', marginBottom: '2px' }}>{label}</div>
    <div style={{ fontSize: '0.6875em', fontWeight: 500, color: 'rgba(255,255,255,0.85)', wordBreak: 'break-word' }}>{value}</div>
  </div>
);

const ForestMainHeader: React.FC<{ title: string; primary: string; accent: string }> = ({ title, primary, accent }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
    <h2 style={{ fontSize: '0.625em', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.16em', color: primary, whiteSpace: 'nowrap', margin: 0 }}>
      {title}
    </h2>
