  ...
  return (
    <div className="resume-paper" style={{
      ...
      <header style={{ textAlign: 'center', marginBottom: '16px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: primary, margin: 0 }}>
          {personal.name || 'YOUR NAME'}
        </h1>
        {personal.title && (
          <p style={{ fontSize: '13px', fontStyle: 'italic', color: '#444', margin: '4px 0 0' }}>
            {personal.title}
          </p>
        )}
        {contactItems.length > 0 && (
          <p style={{ fontSize: '11.5px', color: '#333', margin: '10px 0 0', lineHeight: 1.8 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
        ...
        {personal.summary && (
          <RiverSection title="Profile">
            <RichContent html={personal.summary} style={{ fontSize: '12.5px', textAlign: 'justify', lineHeight: 1.8, color: text }} />
          </RiverSection>
        )}
              ...
              {experience.map(exp => (
                <div key={exp.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <span style={{ fontSize: '13px', fontWeight: 700, color: primary }}>
                      {exp.role}{exp.role && exp.company ? ' — ' : ''}{exp.company}
                    </span>
                    <span style={{ fontSize: '11.5px', color: '#555', flexShrink: 0, marginLeft: '12px' }}>
                  ...
                  {exp.bullets.filter(b => b).length > 0 && (
                    <ul style={{ paddingLeft: '22px', marginTop: '6px', marginBottom: 0 }}>
                      {exp.bullets.filter(b => b).map((bullet, i) => (
                        <li key={i} style={{ fontSize: '12.5px', lineHeight: 1.7, color: text, marginBottom: '2px' }}><RichContent html={bullet} /></li>
                      ))}
                    </ul>
                  )}
                </div>
              ...
              {education.map(edu => (
                <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    {edu.school && (
                      <div style={{ fontSize: '12.5px', fontStyle: 'italic', color: '#444', marginTop: '1px' }}>
                        {edu.school}{edu.gpa ? <span style={{ fontStyle: 'normal', color: '#333', marginLeft: '8px' }}>· GPA {edu.gpa}</span> : ''}
                      </div>
                    )}
                  </div>
                  <span style={{ fontSize: '11.5px', color: '#555', whiteSpace: 'nowrap', marginLeft: '12px', flexShrink: 0 }}>
        ...
        {skills.length > 0 && (
          <RiverSection title="Skills">
            <p style={{ fontSize: '12.5px', lineHeight: 1.8, color: text }}>
              {skills.map((s, i) => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {projects.map(p => (
                <div key={p.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <span style={{ fontSize: '13px', fontWeight: 700, fontStyle: 'italic', color: primary }}>{p.title}</span>
                  {p.url && <span style={{ fontSize: '11.5px', color: '#666', marginLeft: '10px' }}>{p.url}</span>}
                  {p.description && <RichContent html={p.description} style={{ fontSize: '12.5px', color: text, marginTop: '3px', lineHeight: 1.7 }} />}
                  {p.tech.length > 0 && (
                    <p style={{ fontSize: '11.5px', color: '#555', marginTop: '2px', fontStyle: 'italic' }}>
          ...
          <RiverSection title="Certifications">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {certifications.map(c => (
                <p key={c.id} style={{ fontSize: '12px', color: text, margin: 0 }}>
        ...
        {languages.length > 0 && (
          <RiverSection title="Languages">
            <p style={{ fontSize: '12.5px', color: text, lineHeight: 1.8 }}>
              {languages.map((l, i) => (
          ...
          <RiverSection key={section.id} title={section.sectionTitle}>
            <ul style={{ paddingLeft: '22px', margin: 0 }}>
              {section.entries.filter(e => e).map((entry, i) => (
                <li key={i} style={{ fontSize: '12.5px', lineHeight: 1.7, color: text, marginBottom: '2px' }}>{entry}</li>
              ))}
            </ul>
          </RiverSection>
        ))}
