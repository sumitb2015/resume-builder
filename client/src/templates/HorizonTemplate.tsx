  ...
  return (
    <div className="resume-paper" style={{ fontFamily: '"Inter", system-ui, sans-serif', backgroundColor: '#fff' }}>
      ...
      <header style={{ background: `linear-gradient(135deg, ${primary} 0%, ${accent} 100%)`, padding: '40px 48px 36px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          {personal.summary && (
            <div style={{ maxWidth: '260px', background: 'rgba(255,255,255,0.12)', borderRadius: '12px', padding: '14px 16px', backdropFilter: 'blur(4px)' }}>
              <p style={{ fontSize: '11.5px', color: 'rgba(255,255,255,0.9)', lineHeight: 1.7 }}>{personal.summary}</p>
            </div>
          )}
        </div>
        {/* Contact row */}
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexWrap: 'wrap', gap: '6px 20px', marginTop: '20px' }}>
          {[personal.email, personal.phone, personal.location, personal.linkedin, personal.website].filter(Boolean).map((v, i) => (
            <span key={i} style={{ fontSize: '11px', color: 'rgba(255,255,255,0.75)', display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 200px', minHeight: '800px' }}>
        {/* MAIN */}
        <div style={{ padding: '36px 40px', borderRight: `3px solid ${accent}20` }}>
          {experience.length > 0 && (
                ...
                <div key={exp.id} style={{ marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid #F1F5F9' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                    <div>
                      ...
                      <span style={{ marginLeft: '8px', fontSize: '12px', color: accent, fontWeight: 600 }}>{exp.company}</span>
                    </div>
                    <span style={{ fontSize: '10.5px', color: '#94A3B8', backgroundColor: '#F8FAFC', padding: '2px 8px', borderRadius: '10px', flexShrink: 0, marginLeft: '8px' }}>
                  ...
                  <ul style={{ paddingLeft: 0, margin: '8px 0 0' }}>
                    {exp.bullets.filter(b => b).map((b, i) => (
                      <li key={i} style={{ display: 'flex', gap: '8px', fontSize: '12.5px', color: '#475569', lineHeight: 1.65, marginBottom: '4px', listStyle: 'none' }}>
                        <span style={{ color: accent, flexShrink: 0, fontWeight: 700, marginTop: '1px' }}>▸</span>{b}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {projects.map(p => (
                <div key={p.id} style={{ marginBottom: '14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '13.5px', fontWeight: 700, color: '#0F172A' }}>{p.title}</span>
                    {p.url && <span style={{ fontSize: '11px', color: accent }}>{p.url}</span>}
                  </div>
                  {p.description && <p style={{ fontSize: '12.5px', color: '#64748B', marginTop: '3px', lineHeight: 1.6 }}>{p.description}</p>}
                  {p.tech.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '6px' }}>
                      {p.tech.map((t, i) => <span key={i} style={{ fontSize: '10.5px', color: accent, background: accent + '15', padding: '2px 8px', borderRadius: '4px', fontWeight: 600 }}>{t}</span>)}
                    </div>
                  )}
                </div>
              ))}
              ...
              {skills.map(s => (
                <div key={s.id} style={{ marginBottom: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ fontSize: '11.5px', fontWeight: 600, color: '#334155' }}>{s.name}</span>
                  </div>
            ...
            <div>
              ...
              {certifications.map(c => (
                <div key={c.id} style={{ marginBottom: '10px', padding: '8px', background: '#fff', borderRadius: '6px', border: '1px solid #E2E8F0' }}>
                  ...
                  {c.date && <div style={{ fontSize: '10px', color: accent, marginTop: '2px', fontWeight: 600 }}>{c.date}</div>}
                </div>
              ))}
            </div>
          )}
