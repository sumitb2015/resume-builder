    ...
    <div className="resume-paper" style={{ fontFamily: '"DM Sans", system-ui, sans-serif', backgroundColor: '#F8FAFC', padding: 0 }}>
        ...
        <div>
          ...
          {personal.title && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
              ...
              <p style={{ fontSize: '0.8125em', color: accent, fontWeight: 600, letterSpacing: '0.04em' }}>{personal.title}</p>
            </div>
          )}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', alignItems: 'flex-end' }}>
          {[personal.email, personal.phone, personal.location, personal.linkedin, personal.website].filter(Boolean).map((v, i) => (
            <span key={i} style={{ fontSize: '0.7188em', color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      {personal.summary && (
        <div style={{ backgroundColor: primary, padding: '16px 32px' }}>
          <RichContent html={personal.summary} style={{ fontSize: '0.7813em', color: 'rgba(255,255,255,0.75)', lineHeight: 1.75, fontStyle: 'italic' }} />
        </div>
      )}
                ...
                <div key={exp.id} style={{ marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid #F1F5F9' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'start', marginBottom: '4px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '0.75em', fontWeight: 600, color: accent, marginTop: '1px' }}>{exp.company}</div>
                    </div>
                    <span style={{ fontSize: '0.6563em', color: '#94A3B8', whiteSpace: 'nowrap', fontWeight: 500, padding: '3px 8px', background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '6px' }}>
                  ...
                  <ul style={{ paddingLeft: 0, margin: '8px 0 0' }}>
                    {exp.bullets.filter(b=>b).map((b,i) => (
                      <li key={i} style={{ display: 'flex', gap: '10px', fontSize: '0.7813em', color: '#475569', lineHeight: 1.65, marginBottom: '5px', listStyle: 'none' }}>
                        <span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: accent, flexShrink: 0, marginTop: '6px' }} /><RichContent html={b} />
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <SlateSec title="Projects" accent={accent}>
              {projects.map(p => (
                <div key={p.id} style={{ marginBottom: '14px', padding: '12px', border: '1px solid #E2E8F0', borderRadius: '8px' }}>
                  ...
                  {p.description && <RichContent html={p.description} style={{ fontSize: '0.7813em', color: '#64748B', marginTop: '4px', lineHeight: 1.6 }} />}
                  {p.tech.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '6px' }}>
                      {p.tech.map((t,i) => <span key={i} style={{ fontSize: '0.6563em', backgroundColor: '#EFF6FF', color: '#2563EB', padding: '2px 8px', borderRadius: '4px', fontWeight: 600 }}>{t}</span>)}
                    </div>
                  )}
                </div>
              ))}
              ...
              {skills.map(s => (
                <div key={s.id} style={{ marginBottom: '9px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ fontSize: '0.7188em', fontWeight: 600, color: primary }}>{s.name}</span>
                  </div>
...
const SlateSideTitle: React.FC<{ title: string; accent: string }> = ({ title, accent }) => (
  <h3 style={{ fontSize: '0.625em', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.16em', color: accent, marginBottom: '12px', paddingBottom: '5px', borderBottom: `2px solid ${accent}40` }}>{title}</h3>
);
