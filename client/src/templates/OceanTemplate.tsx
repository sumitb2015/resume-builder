      ...
      <header style={{ backgroundColor: primary, padding: '32px 44px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1, marginRight: '32px' }}>
            ...
            {personal.summary && (
              <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, margin: '12px 0 0', maxWidth: '380px' }}>
                {personal.summary}
              </p>
            )}
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {projects.map(p => (
                  <div key={p.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    ...
                    {p.description && <p style={{ fontSize: '12px', color: text, margin: '0 0 4px', lineHeight: 1.6 }}>{p.description}</p>}
                    {p.tech.length > 0 && (
