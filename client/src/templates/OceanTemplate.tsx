      ...
      <header style={{ backgroundColor: primary, padding: '11mm 15mm' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1, marginRight: '32px' }}>
            ...
            {personal.title && (
              <p style={{ fontSize: '12px', color: accent, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.1em', margin: '6px 0 0' }}>
                {personal.title}
              </p>
            )}
            {personal.summary && (
              <RichContent html={personal.summary} style={{ fontSize: '12px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, margin: '12px 0 0', maxWidth: '380px' }} />
            )}
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px', flexShrink: 0 }}>
              {contactItems.map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.75)', textAlign: 'right' }}>{item}</span>
                ...
                {experience.map(exp => (
                  <div key={exp.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3px' }}>
                      <div>
                        <div style={{ fontSize: '13.5px', fontWeight: 700, color: primary }}>{exp.role}</div>
                        <div style={{ fontSize: '11.5px', fontWeight: 600, color: accent, marginTop: '1px' }}>{exp.company}</div>
                      </div>
                      <span style={{ fontSize: '10.5px', color: '#94A3B8', whiteSpace: 'nowrap', marginLeft: '12px', flexShrink: 0 }}>
                    ...
                    {exp.bullets.filter(b => b).length > 0 && (
                      <ul style={{ marginTop: '6px', display: 'flex', flexDirection: 'column', gap: '4px', paddingLeft: '0' }}>
                        {exp.bullets.filter(b => b).map((bullet, i) => (
                          <li key={i} style={{ display: 'flex', gap: '10px', fontSize: '12.5px', color: text, lineHeight: 1.65, listStyle: 'none' }}>
                ...
                {projects.map(p => (
                  <div key={p.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '3px' }}>
                      <span style={{ fontSize: '13px', fontWeight: 700, color: primary }}>{p.title}</span>
                      {p.url && <span style={{ fontSize: '11px', color: accent }}>{p.url}</span>}
                    </div>
                    {p.description && <RichContent html={p.description} style={{ fontSize: '12px', color: text, margin: '0 0 4px', lineHeight: 1.6 }} />}
                    {p.tech.length > 0 && (
                      <p style={{ fontSize: '11px', color: accent, margin: 0 }}>{p.tech.join(' · ')}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          {skills.length > 0 && (
            <OceanSideSection title="Skills" primary={primary} accent={accent}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {skills.map(s => (
                  <div key={s.id}>
                    <div style={{ fontSize: '11px', fontWeight: 600, color: primary, marginBottom: '4px' }}>{s.name}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                {languages.map(l => (
                  <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <span style={{ fontSize: '11.5px', fontWeight: 600, color: primary }}>{l.language}</span>
                    <span style={{ fontSize: '10.5px', color: '#94A3B8' }}>{l.proficiency}</span>
                  </div>
                ))}
              </div>
            </OceanSideSection>
            ...
            <OceanSideSection title="Certifications" primary={primary} accent={accent}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {certifications.map(c => (
                  <div key={c.id}>
                    <div style={{ fontSize: '11px', fontWeight: 700, color: primary }}>{c.name}</div>
                    {c.issuer && <div style={{ fontSize: '10px', color: '#94A3B8', marginTop: '1px' }}>{c.issuer}</div>}
                    {c.date && <div style={{ fontSize: '10px', color: accent, marginTop: '1px' }}>{c.date}</div>}
                  </div>
                ))}
              </div>
            </OceanSideSection>
