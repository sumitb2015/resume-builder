    ...
    <div className="resume-paper" style={{ fontFamily: '"Poppins", system-ui, sans-serif', backgroundColor: '#fff', display: 'flex', flexDirection: 'column', padding: 0 }}>
      ...
      <div style={{ backgroundColor: primary, padding: '12mm 15mm 10mm', position: 'relative' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '0.6875em', fontWeight: 700, color: accent, letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: '16px' }}>
            ◆ {personal.title || 'Professional Resume'}
          </div>
          <h1 style={{ fontSize: '3.25em', fontWeight: 900, color: '#FAFAFA', letterSpacing: '-0.04em', lineHeight: 0.95, marginBottom: '24px' }}>
            {(personal.name || 'YOUR NAME').split(' ').map((word, i) => (
              <div key={i}>{word}</div>
            ))}
          </h1>
          {personal.summary && (
            <RichContent html={personal.summary} style={{ fontSize: '0.7813em', color: 'rgba(255,255,255,0.5)', lineHeight: 1.8, maxWidth: '480px' }} />
          )}
        </div>
      </div>
      ...
      <div style={{ backgroundColor: accent, padding: '4mm 15mm', display: 'flex', gap: '24px', flexWrap: 'wrap', alignItems: 'center' }}>
        {[personal.email, personal.phone, personal.location, personal.linkedin, personal.website].filter(Boolean).map((v, i) => (
          <span key={i} style={{ fontSize: '0.6875em', fontWeight: 600, color: '#fff', letterSpacing: '0.02em' }}>{v}</span>
        ))}
      </div>
              ...
              {experience.map(exp => (
                <div key={exp.id} style={{ marginBottom: '22px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
                    <div>
                      <div style={{ fontSize: '0.9375em', fontWeight: 800, color: primary, letterSpacing: '-0.02em' }}>{exp.role}</div>
                      <div style={{ fontSize: '0.75em', fontWeight: 700, color: accent, textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '2px' }}>{exp.company}</div>
                    </div>
                    <span style={{ fontSize: '0.625em', color: '#A1A1AA', backgroundColor: '#F4F4F5', padding: '3px 8px', borderRadius: '4px', flexShrink: 0, marginLeft: '10px', fontWeight: 600 }}>
                  ...
                  <ul style={{ paddingLeft: 0, margin: '8px 0 0' }}>
                    {exp.bullets.filter(b=>b).map((b,i) => (
                      <li key={i} style={{ display: 'flex', gap: '10px', fontSize: '0.7813em', color: '#3F3F46', lineHeight: 1.7, marginBottom: '5px', listStyle: 'none' }}>
              ...
              {projects.map(p => (
                <div key={p.id} style={{ marginBottom: '14px', borderLeft: `3px solid ${accent}`, paddingLeft: '14px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <span style={{ fontSize: '0.8438em', fontWeight: 800, color: primary }}>{p.title}</span>
                    {p.url && <span style={{ fontSize: '0.6875em', color: accent }}>{p.url}</span>}
                  </div>
                  {p.description && <RichContent html={p.description} style={{ fontSize: '0.7813em', color: '#52525B', marginTop: '4px', lineHeight: 1.6 }} />}
                  {p.tech.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '6px' }}>
                      {p.tech.map((t,i) => <span key={i} style={{ fontSize: '0.6563em', backgroundColor: primary, color: '#fff', padding: '2px 8px', borderRadius: '4px', fontWeight: 600 }}>{t}</span>)}
                    </div>
                  )}
                </div>
              ))}
            ...
            <BoldSec title="Education" accent={accent} primary={primary}>
              {education.map(edu => (
                <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div>
                    <div style={{ fontSize: '0.875em', fontWeight: 800, color: primary }}>{edu.school}</div>
                    <div style={{ fontSize: '0.75em', color: '#71717A', marginTop: '2px' }}>{[edu.degree, edu.field].filter(Boolean).join(' in ')}</div>
                  </div>
                  <span style={{ fontSize: '0.6875em', color: '#A1A1AA', flexShrink: 0 }}>{edu.endDate}{edu.gpa ? ` · GPA ${edu.gpa}` : ''}</span>
                </div>
              ))}
            </BoldSec>
          )}
          ...
          {skills.length > 0 && (
            <div style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {skills.map(s => (
                  <div key={s.id}>
                    <div style={{ fontSize: '0.75em', fontWeight: 600, color: primary, marginBottom: '4px' }}>{s.name}</div>
            ...
            <div style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              {languages.map(l => (
                <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75em', lineHeight: 2, color: primary }}>
                  ...
                  <span style={{ color: '#A1A1AA', fontSize: '0.6875em' }}>{l.proficiency}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              {certifications.map(c => (
                <div key={c.id} style={{ marginBottom: '10px' }}>
                  ...
                  {c.issuer && <div style={{ fontSize: '0.6875em', color: '#71717A' }}>{c.issuer}</div>}
...
const BoldSideTitle: React.FC<{ title: string; accent: string }> = ({ title, accent }) => (
  <h3 style={{ fontSize: '0.625em', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.18em', color: accent, marginBottom: '12px', paddingBottom: '5px', borderBottom: `2px solid ${accent}` }}>{title}</h3>
);
