      ...
      <header style={{ backgroundColor: primary, padding: '36px 52px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            ...
            {personal.title && (
              <p style={{ fontSize: '0.75em', color: '#AAAAAA', textTransform: 'uppercase', letterSpacing: '0.14em', margin: '6px 0 0' }}>
                {personal.title}
              </p>
            )}
          </div>
        ...
        {contactItems.length > 0 && (
          <div style={{ marginTop: '16px', fontSize: '0.6563em', color: '#CCCCCC', lineHeight: 1.8 }}>
                ...
                <div key={exp.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '0.75em', fontWeight: 600, color: '#555555', marginTop: '1px' }}>{exp.company}</div>
                    </div>
                    <span style={{ fontSize: '0.6563em', color: '#888888', whiteSpace: 'nowrap', marginLeft: '12px', flexShrink: 0 }}>
              ...
              {education.map(edu => (
                <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.75em', fontStyle: 'italic', color: '#555555', marginTop: '2px' }}>
              ...
              {projects.map(p => (
                <div key={p.id} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '3px' }}>
                    ...
                    {p.url && <span style={{ fontSize: '0.6875em', color: '#666' }}>{p.url}</span>}
                  </div>
                  {p.description && <RichContent html={p.description} style={{ fontSize: '0.7813em', color: text, margin: '0 0 3px', lineHeight: 1.65 }} />}
                  {p.tech.length > 0 && (
                    <p style={{ fontSize: '0.6875em', color: '#555555', margin: 0, fontStyle: 'italic' }}>
                      ({p.tech.join(', ')})
                    </p>
                  )}
                </div>
              ...
              <NightSection title="Certifications" primary={primary}>
                {certifications.map(c => (
                  <div key={c.id} style={{ marginBottom: '8px' }}>
                    ...
                    {(c.issuer || c.date) && (
                      <div style={{ fontSize: '0.7188em', color: '#888888', marginTop: '1px' }}>
