      ...
      <header style={{ padding: '0' }}>
        <div style={{ background: primary, padding: '13mm 16mm 11mm', position: 'relative' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'inline-block', backgroundColor: accent, padding: '3px 10px', marginBottom: '12px' }}>
              <span style={{ fontSize: '0.625em', fontWeight: 800, color: '#fff', letterSpacing: '0.2em', textTransform: 'uppercase' }}>{personal.title || 'Professional'}</span>
            </div>
            <h1 style={{ fontSize: '3em', fontWeight: 900, color: '#fff', letterSpacing: '-0.04em', lineHeight: 1.0, marginBottom: '0' }}>
        ...
        <div style={{ backgroundColor: accent, padding: '3mm 16mm', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          {[personal.email, personal.phone, personal.location, personal.linkedin, personal.website].filter(Boolean).map((v, i) => (
            <span key={i} style={{ fontSize: '0.6875em', fontWeight: 600, color: '#fff', letterSpacing: '0.03em' }}>{v}</span>
          ))}
        </div>
      </header>
        ...
        <div style={{ padding: '12mm 11mm', borderRight: `3px solid ${primary}` }}>
          {personal.summary && (
            <div style={{ marginBottom: '28px' }}>
              <RichContent html={personal.summary} style={{ fontSize: '0.875em', lineHeight: 1.8, color: '#27272A', borderLeft: `4px solid ${accent}`, paddingLeft: '16px', fontStyle: 'italic' }} />
            </div>
          )}
          {experience.length > 0 && (
                ...
                <div key={exp.id} style={{ marginBottom: '22px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '0.75em', fontWeight: 700, color: accent, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{exp.company}</div>
                    </div>
                    <span style={{ fontSize: '0.6563em', color: '#71717A', backgroundColor: '#F4F4F5', padding: '3px 8px', borderRadius: '4px', flexShrink: 0, marginLeft: '10px' }}>
                  ...
                  <ul style={{ paddingLeft: 0, margin: '8px 0 0' }}>
                    {exp.bullets.filter(b=>b).map((b,i) => (
                      <li key={i} style={{ display: 'flex', gap: '8px', fontSize: '0.7813em', color: '#52525B', lineHeight: 1.65, marginBottom: '4px', listStyle: 'none' }}>
            ...
            <MagSection title="Projects" primary={primary} accent={accent}>
              {projects.map(p => (
                <div key={p.id} style={{ marginBottom: '14px', padding: '12px', border: `2px solid ${primary}10`, borderRadius: '6px' }}>
                  ...
                  {p.description && <RichContent html={p.description} style={{ fontSize: '0.7813em', color: '#52525B', margin: '4px 0' }} />}
                  {p.tech.length > 0 && (
              ...
              {education.map(edu => (
                <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.75em', color: '#71717A' }}>{[edu.degree, edu.field].filter(Boolean).join(' in ')}</div>
                  </div>
                ...
                {skills.map(s => (
                  <div key={s.id}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                      <span style={{ fontSize: '0.7188em', fontWeight: 700, color: primary }}>{s.name}</span>
                    </div>
            ...
            <div>
              ...
              {certifications.map(c => (
                <div key={c.id} style={{ marginBottom: '10px' }}>
                  ...
                  {c.issuer && <div style={{ fontSize: '0.6875em', color: '#71717A', marginTop: '1px' }}>{c.issuer}</div>}
                  {c.date && <div style={{ fontSize: '0.6563em', color: accent, marginTop: '2px', fontWeight: 600 }}>{c.date}</div>}
                </div>
              ))}
            </div>
          )}
...
const MagSideTitle: React.FC<{ title: string; primary: string; accent: string }> = ({ title, accent }) => (
  <h3 style={{ fontSize: '0.625em', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.16em', color: accent, marginBottom: '12px', paddingBottom: '5px', borderBottom: `2px solid ${accent}` }}>{title}</h3>
);
