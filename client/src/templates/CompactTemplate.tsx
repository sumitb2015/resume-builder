      ...
      <header style={{ borderBottom: `2px solid ${accent}`, paddingBottom: '10px', marginBottom: '14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            ...
            {personal.title && <p style={{ fontSize: '0.75em', color: accent, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{personal.title}</p>}
          </div>
          <div style={{ textAlign: 'right', fontSize: '0.6563em', color: '#64748B', lineHeight: 1.8 }}>
          ...
          {experience.map(exp => (
            <div key={exp.id} style={{ marginBottom: '12px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <span style={{ fontWeight: 700, fontSize: '0.75em', color: '#0F172A' }}>{exp.role}</span>
                <span style={{ fontSize: '0.625em', color: '#94A3B8', flexShrink: 0, marginLeft: '8px' }}>{exp.startDate}{exp.startDate||exp.endDate?' – ':''}{exp.isCurrent?'Present':exp.endDate}</span>
              </div>
            ...
            <CSection title="Education" accent={accent} config={config}>
              {education.map(edu => (
                <div key={edu.id} style={{ marginBottom: '8px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  ...
                  <div style={{ fontSize: '0.6563em', color: '#64748B' }}>{[edu.degree, edu.field].filter(Boolean).join(' in ')}</div>
                  <div style={{ fontSize: '0.625em', color: '#94A3B8' }}>{edu.endDate}{edu.gpa ? ` · GPA ${edu.gpa}` : ''}</div>
                </div>
              ))}
            </CSection>
          )}
            ...
            <CSection title="Certifications" accent={accent} config={config}>
              {certifications.map(c => (
                <div key={c.id} style={{ marginBottom: '6px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  ...
                  {c.issuer && <div style={{ fontSize: '0.625em', color: '#64748B' }}>{c.issuer}{c.date ? ` · ${c.date}` : ''}</div>}
                </div>
              ))}
            </CSection>
          )}
