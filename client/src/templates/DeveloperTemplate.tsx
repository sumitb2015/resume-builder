      ...
      <header style={{ backgroundColor: primary, padding: '36px 48px 28px', color: '#fff' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <h1 style={{ fontSize: '1.875em', fontWeight: 800, color: '#F0F6FC', letterSpacing: '-0.02em', lineHeight: 1.1, marginBottom: '6px' }}>{personal.name || 'YOUR NAME'}</h1>
            {personal.title && <p style={{ fontSize: '0.8125em', color: accent, fontWeight: 500, fontFamily: 'monospace' }}>// {personal.title}</p>}
          </div>
          <div style={{ textAlign: 'right', fontFamily: 'monospace', fontSize: '0.6875em', color: '#7D8590', lineHeight: 2 }}>
        ...
        {personal.summary && (
          <div style={{ marginTop: '18px', padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.04)', borderLeft: `3px solid ${accent}`, borderRadius: '0 6px 6px 0', fontFamily: 'monospace', fontSize: '0.75em', color: '#8B949E', lineHeight: 1.7 }}>
                ...
                <div key={exp.id} style={{ marginBottom: '20px', padding: '14px', backgroundColor: '#F8FAFC', borderRadius: '8px', borderLeft: `3px solid ${accent}`, breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                    <div>
                      ...
                      <span style={{ fontSize: '0.8125em', fontWeight: 600, color: accent }}>{exp.company}</span>
                    </div>
                    <span style={{ fontFamily: 'monospace', fontSize: '0.6563em', color: '#94A3B8', flexShrink: 0, marginLeft: '10px' }}>
              ...
              {projects.map(p => (
                <div key={p.id} style={{ marginBottom: '14px', padding: '12px', border: '1px solid #E2E8F0', borderRadius: '8px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <span style={{ fontSize: '0.8438em', fontWeight: 700, color: '#0F172A', fontFamily: 'monospace' }}>{p.title}</span>
                    {p.url && <span style={{ fontSize: '0.6563em', color: accent, fontFamily: 'monospace' }}>{p.url}</span>}
                  </div>
                  {p.description && <p style={{ fontSize: '0.75em', color: '#64748B', margin: '6px 0' }}>{p.description}</p>}
                  {p.tech.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                      {p.tech.map((t,i) => <span key={i} style={{ fontFamily: 'monospace', fontSize: '0.6563em', backgroundColor: accent+'20', color: accent, padding: '2px 8px', borderRadius: '4px', fontWeight: 600 }}>{t}</span>)}
                    </div>
                  )}
                </div>
              ))}
              ...
              {education.map(edu => (
                <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.75em', color: '#64748B', marginTop: '2px' }}>{[edu.degree, edu.field].filter(Boolean).join(' in ')}</div>
                  </div>
                  <span style={{ fontFamily: 'monospace', fontSize: '0.6875em', color: '#94A3B8', flexShrink: 0 }}>{edu.endDate}</span>
                </div>
              ))}
            </DevSection>
          )}
          ...
          {skills.length > 0 && (
            <div style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {skills.map(s => (
                  <div key={s.id} style={{ fontFamily: 'monospace', fontSize: '0.7188em', color: '#334155', backgroundColor: '#F1F5F9', padding: '4px 8px', borderRadius: '4px', display: 'flex', justifyContent: 'space-between' }}>
                    <span>{s.name}</span>
            ...
            <div style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              {certifications.map(c => (
                <div key={c.id} style={{ marginBottom: '8px', fontSize: '0.7188em' }}>
                  ...
                  {c.issuer && <div style={{ color: '#94A3B8', fontSize: '0.6563em' }}>{c.issuer}</div>}
                  {c.date && <div style={{ color: accent, fontSize: '0.625em', fontFamily: 'monospace' }}>{c.date}</div>}
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              {languages.map(l => (
                <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7188em', lineHeight: 2 }}>
                  ...
                  <span style={{ color: '#94A3B8', fontFamily: 'monospace', fontSize: '0.625em' }}>{l.proficiency}</span>
                </div>
              ))}
            </div>
          )}
...
const DevSideTitle: React.FC<{ title: string; accent: string }> = ({ title, accent }) => (
  <h3 style={{ fontFamily: 'monospace', fontSize: '0.625em', fontWeight: 700, color: accent, textTransform: 'lowercase', letterSpacing: '0.08em', marginBottom: '10px', paddingBottom: '5px', borderBottom: `1px solid ${accent}30` }}>
    &gt; {title}
  </h3>
);
