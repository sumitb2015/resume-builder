  ...
  return (
    <div className="resume-paper" style={{
      ...
      <header style={{ marginBottom: '32px' }}>
        ...
        {personal.title && (
          <p style={{ fontSize: '0.9375em', fontWeight: 400, color: '#888', letterSpacing: '0.01em', marginBottom: '24px' }}>
            {personal.title}
          </p>
        )}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 24px' }}>
          {[personal.email, personal.phone, personal.location, personal.linkedin, personal.website].filter(Boolean).map((v, i) => (
            <span key={i} style={{ fontSize: '0.7188em', color: '#777', letterSpacing: '0.02em' }}>{v}</span>
          ))}
        </div>
      </header>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
                ...
                <div key={exp.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '4px' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px' }}>
                      <span style={{ fontSize: '0.875em', fontWeight: 600, color: '#0a0a0a', letterSpacing: '-0.01em' }}>{exp.company}</span>
                      {exp.role && <span style={{ fontSize: '0.75em', color: '#888', fontWeight: 400 }}>{exp.role}</span>}
                    </div>
                    <span style={{ fontSize: '0.7188em', color: '#aaa', flexShrink: 0, marginLeft: '12px' }}>
                  ...
                  {exp.bullets.filter(b => b).length > 0 && (
                    <ul style={{ display: 'flex', flexDirection: 'column', gap: '6px', paddingLeft: '0', marginTop: '8px' }}>
                      {exp.bullets.filter(b => b).map((bullet, i) => (
                        <li key={i} style={{ display: 'flex', gap: '12px', fontSize: '0.7813em', color: '#555', lineHeight: 1.7, listStyle: 'none' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {education.map(edu => (
                <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: '0.875em', fontWeight: 600, color: '#0a0a0a' }}>{edu.school}</div>
                    <div style={{ fontSize: '0.7813em', color: '#777', marginTop: '2px' }}>
                      {[edu.degree, edu.field].filter(Boolean).join(' in ')}
                      {edu.gpa && <span style={{ marginLeft: '8px', color: '#444', fontWeight: 500 }}>· GPA {edu.gpa}</span>}
                    </div>
                  </div>
                  <span style={{ fontSize: '0.7188em', color: '#aaa', whiteSpace: 'nowrap', marginLeft: '12px', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', columnGap: '32px', rowGap: '8px' }}>
              {skills.map(s => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontSize: '0.8125em', color: '#222', fontWeight: 500 }}>{s.name}</span>
              ...
              {projects.map(p => (
                <div key={p.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <span style={{ fontSize: '0.8438em', fontWeight: 600, color: '#0a0a0a' }}>{p.title}</span>
                    {p.url && <span style={{ fontSize: '0.7188em', color: '#aaa' }}>{p.url}</span>}
                  </div>
                  {p.description && <RichContent html={p.description} style={{ fontSize: '0.7813em', color: '#555', marginTop: '4px', lineHeight: 1.7 }} />}
                  {p.tech.length > 0 && (
                    <p style={{ fontSize: '0.7188em', color: '#999', marginTop: '3px' }}>{p.tech.join(' · ')}</p>
                  )}
                </div>
              ))}
            </div>
              ...
              <MinimalSection title="Languages" accent={accent} config={config}>
                {languages.map(l => (
                  <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8125em', lineHeight: 2 }}>
                    ...
                    <span style={{ color: '#aaa', fontSize: '0.75em' }}>{l.proficiency}</span>
                  </div>
                ))}
              </MinimalSection>
            )}
              ...
              <MinimalSection title="Certifications" accent={accent} config={config}>
                {certifications.map(c => (
                  <div key={c.id} style={{ marginBottom: '8px' }}>
                    ...
                    {c.issuer && <div style={{ fontSize: '0.75em', color: '#888' }}>{c.issuer}{c.date ? ` · ${c.date}` : ''}</div>}
                  </div>
                ))}
              </MinimalSection>
            )}
        ...
        {custom && custom.length > 0 && custom.map(sec => (
          <MinimalSection key={sec.id} title={sec.sectionTitle || 'Custom Section'} accent={accent} config={config}>
              ...
              {sec.entries.filter(e => e).map((entry, i) => (
                <div key={i} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                  <RichContent 
                    ...
                    style={{ fontSize: '0.8125em', lineHeight: 1.85, color: '#444' }} 
                  />
                </div>
              ))}
            </div>
  ...
  <section style={{ marginBottom: '24px' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
      <h2 style={{
        ...
        color: accent, whiteSpace: 'nowrap', fontFamily: config.fonts.heading,
      }}>
        {title}
      </h2>
