    ...
    >
      ...
      <aside style={{
        ...
        <div>
          ...
          {personal.title && (
            <p style={{ fontSize: '0.6875em', fontWeight: 500, color: accent, textTransform: 'uppercase', letterSpacing: '0.1em', lineHeight: 1.4 }}>
              {personal.title}
            </p>
          )}
        </div>
              ...
              {skills.map(s => (
                <div key={s.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                    <span style={{ fontSize: '0.7188em', fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{s.name}</span>
                  </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {languages.map(l => (
                <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.7188em', fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{l.language}</span>
                  <span style={{
                    fontSize: '0.625em', fontWeight: 600, color: primary,
          ...
          <SideSection title="Certifications" accent={accent} config={config}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {certifications.map(c => (
                <div key={c.id}>
                  <div style={{ fontSize: '0.7188em', fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{c.name}</div>
                  {c.issuer && <div style={{ fontSize: '0.6563em', color: 'rgba(255,255,255,0.5)', marginTop: '1px' }}>{c.issuer}</div>}
                </div>
              ))}
            </div>
          </SideSection>
              ...
              {experience.map((exp) => (
                <div key={exp.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3px' }}>
                    <div>
                      <div style={{ fontSize: '0.875em', fontWeight: 700, color: '#111827', letterSpacing: '-0.01em' }}>{exp.role}</div>
                      <div style={{ fontSize: '0.75em', fontWeight: 600, color: accent, marginTop: '1px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{exp.company}</div>
                    </div>
                    <div style={{
                      fontSize: '0.6563em', fontWeight: 600, color: '#6B7280', whiteSpace: 'nowrap',
                  ...
                  {exp.bullets.filter(b => b).length > 0 && (
                    <ul style={{ marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '5px', paddingLeft: '0' }}>
                      {exp.bullets.filter(b => b).map((bullet, i) => (
                        <li key={i} style={{ display: 'flex', gap: '10px', fontSize: '0.7813em', color: '#4B5563', lineHeight: 1.65, listStyle: 'none' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {education.map((edu) => (
                <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: '0.6563em', color: '#9CA3AF', whiteSpace: 'nowrap', marginLeft: '12px', flexShrink: 0 }}>
              ...
              {projects.map(p => (
                <div key={p.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <span style={{ fontSize: '0.8438em', fontWeight: 700, color: '#111827' }}>{p.title}</span>
                    {p.url && <span style={{ fontSize: '0.6875em', color: accent }}>{p.url}</span>}
                  </div>
                  {p.description && <RichContent html={p.description} style={{ fontSize: '0.7813em', color: '#4B5563', marginTop: '4px', lineHeight: 1.6 }} />}
                  {p.tech.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '6px' }}>
                      {p.tech.map((t, i) => (
                        <span key={i} style={{ fontSize: '0.6563em', backgroundColor: '#EEF2FF', color: accent, padding: '2px 8px', borderRadius: '4px', fontWeight: 600 }}>{t}</span>
                      ))}
                    </div>
                  )}
                </div>
...
const ContactRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div>
    <div style={{ fontSize: '0.5938em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.35)', marginBottom: '2px' }}>{label}</div>
    <div style={{ fontSize: '0.7188em', fontWeight: 500, color: 'rgba(255,255,255,0.85)', wordBreak: 'break-word' }}>{value}</div>
  </div>
);

const MainSectionTitle: React.FC<{ title: string; accent: string; config: TemplateConfig }> = ({ title, accent, config }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
    <h2 style={{ fontFamily: config.fonts.heading, fontSize: '0.6875em', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.12em', color: accent, whiteSpace: 'nowrap' }}>
      {title}
    </h2>
