  ...
  return (
    <section style={{ padding: isMobile ? '60px 20px' : '100px 48px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: isMobile ? '14px' : '16px', color: 'var(--color-ui-text-muted)', marginBottom: isMobile ? '48px' : '72px' }}>
          No fuss, no friction. Most users create a polished resume in under 15 minutes.
        </p>

        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '48px' : '0', position: 'relative' }}>
          {/* Connecting line */}
          {!isMobile && (
            <div style={{
          ...
          {STEPS.map((step, i) => (
            <div key={i} style={{ flex: 1, textAlign: 'center', padding: isMobile ? '0' : '0 24px', position: 'relative', zIndex: 1 }}>
              {/* Icon circle */}
              <div style={{
